    ...
    def didWin(self):
        ...
        if won:
            display.display_message(
                f'congratulations. you won the game. The game lasted for {round(self.elapsed)} seconds and you used {self.moves} moves.')
            quit()
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP and self.y < 2:
                    ...
                    display.display_message(
                        f'This game lasted for {round(self.elapsed)} seconds and you finished with {self.moves} moves.')
                    pygame.quit()
                    quit()
